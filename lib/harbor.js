...
var debug = require('debug')('harbor')
...
Harbor.prototype.claim = function (name, cb) {
  ...
  checkPort.call(this, this.min, function (err, port) {
    ...
    debug('%s claims port %d', name, port);
...
function checkPort (num, cb) {
  ...
  // how to handle a valid port
  function found () {
    debug('found port %d', num);
    cb(null, num);
  }
